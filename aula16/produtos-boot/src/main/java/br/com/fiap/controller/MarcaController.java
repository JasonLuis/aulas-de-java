...
public class MarcaController {
	...
	public ResponseEntity<List<MarcaModel>> findAll(Model model) {
		List<MarcaModel> marca = repository.findAll();
	...
	public ResponseEntity<MarcaModel> findById(@PathVariable("id") long id) {
		...
		MarcaModel marcas = repository.findById(id).get();
	...
	public ResponseEntity save(@RequestBody @Valid MarcaModel marcaModel, BindingResult bindingResult) {
		...
		@Valid
		MarcaModel marcas = repository.save(marcaModel);
		
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				.buildAndExpand(marcas.getIdMarca()).toUri();
